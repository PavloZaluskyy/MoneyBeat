...
export class ReportComponent implements OnInit {
  ...
  showReportbyReceipt: {name: string; value: number}[] | null = []
    ...
    getDifferenceDayBetweenTwoDates(firstDate: any = this.currentDateFrom, secondDate: any = this.currentDate){
      let date1, date2;
      if (!firstDate.year || !secondDate.year) {
         ...
         date2 = new Date(`${secondDate.year}-${secondDate.month}-${secondDate.day}`);
      }
    ...
    getTotalAmound(){
      console.log(this.showReportbyReceipt);
      let sum = 0;
    ...
    if(startDate.year) {
      ...
      console.log(this.getCategoriesAndTZotalAmound(arr));
      this.showReportbyReceipt = this.getCategoriesAndTZotalAmound(arr)
    }

    getCategoriesAndTZotalAmound(receipts: Receipt[]):
