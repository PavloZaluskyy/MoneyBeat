...
export class GoodStatisticComponent implements OnInit {
  ...
  getGoodsbySelectGood(){
    if (this.viewGoods.length) {
      let arr = this.viewGoods.map((item: any) => {
        return { name: item.name, value: item.price, quantity: item.quantity, date: item.date };
      })
  ...
  convertDateForPie() {
    if(this.viewGoods && this.viewGoods.length) {
      this.chartData.data = this.goodData.map((item: {name: string, value: number, quanity: number, color: string, date: string}) => {return [item.date, item.value];});
    }
  }
  convertDateForPieQuanity() {
    if(this.viewGoods && this.viewGoods.length) {
      this.chartDataQuanity.data = this.goodData.map((item: {name: string, value: number, quantity: number, color: string, date: string}) => {return [item.date, item.quantity];});
    }
  }
}
