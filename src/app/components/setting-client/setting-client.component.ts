...
export class SettingClientComponent implements OnInit {
  ...
  messages: string = '';
  isValidation: { name: boolean; moneyInCard: boolean } = {
    name: true,
    moneyInCard: true,
  };
  constructor(
  ...
  save() {
    if (this.validation()) {
      this._client.setClient({
        ...
        moneyInCard: this.client.moneyInCard,
        pin: this.client.pin ? this.client.pin : 1008,
        checkSign: this.client.checkSign,
      });
