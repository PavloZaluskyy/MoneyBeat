...
export class TopStoreComponent implements OnInit {
  @ViewChild('closebutton') closebutton: ElementRef | undefined;
    ...
    getReceiptByDateAndCategory(startDate: any, endDate: any) {
      ...
      console.log(this.getDateForStore(arr));
    ...
    sortDate():Receipt[]{
      return this.viewReceipt.sort((a,b) => a.totalAmaund > b.totalAmaund ? -1 : 0)
    }
    firstDateOfMonth() {
      let date = new Date();
    ...
    convertDateForPieQuanity() {
      if (this.viewReceipt && this.viewReceipt.length) {
        this.DateCategoryGoogle = this.viewReceipt.map(
          (item: Receipt) => {
    ...
    ngOnInit(): void {
      ...
      console.log(this.receiptAll);
      this.viewReceipt=this.getDateForStore(this.receiptAll);
      this.convertDateForPieQuanity()
      this.viewReceipt=this.sortDate()
      // console.log(this.getDateForStore());
      console.log(this.DateCategoryGoogle);
    ...
    getDateForStore(arr: Receipt[]): Receipt[]{
      const outputData = arr.reduce((acc: any, curr) => {
        const existingEntry = acc.find((item: any) => item.nameStore === curr.nameStore);
        if (existingEntry) {
